...
begin
	...
	until eof;}
	assign(input,'2017zz.in');reset(input);
	repeat
		readln(str);
		str:=str+',';
		section:=1;
	...
	until eof;    }
	close(input);
	assign(output,'javascriptzz.out');rewrite(output);
	for i:=170000 to 199999 do if data[i].name<>'' then writeln('new Student(',data[i].year,', ',data[i].class+10,', ',data[i].number,', ''',data[i].name,''', ''',i+220000000-1000,'.png''),');
	close(output);
end.
